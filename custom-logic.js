...
async function renderPage(pageNum) {
    ...
    const scale = Math.min((containerWidth - 40) / viewport.width, 1.8); // Max scale of 1.8 for readability
    ...
    wrapper.id = id;
    wrapper.style.width = `${Math.ceil(scaledViewport.width)}px`;
    wrapper.style.height = `${Math.ceil(scaledViewport.height)}px`;
...
function renderResults(matches) {
    ...
    matches.forEach((m, idx) => {
        ...
        btn.innerHTML = `<strong>Page ${item.pageNum}${score}</strong><br>${escapeHtml(preview)}`;
        
        btn.addEventListener('click', async () => {
            // Scroll to the page
            const pageElement = document.getElementById(`page-${item.pageNum}`);
            if (pageElement) {
                ...
                const scale = Math.min((containerWidth - 40) / viewport.width, 1.8);
                const scaledViewport = page.getViewport({ scale });
                highlightMatch(pageElement, scaledViewport, item);
            }
        });
...
// Handle window resize to adjust PDF pages
window.addEventListener('resize', () => {
    if (gPdf && renderedPages.size > 0) {
        // Re-render all pages on resize to adjust scale
        renderedPages.clear();
        pdfContainer.innerHTML = '';
        renderAllPages();
    }
